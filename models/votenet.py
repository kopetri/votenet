...
class VoteNetModule(LightningModule):
    ...
    def visualize_prediction(self, batch, end_points, segmentation_label, objectness_label, adjacent_matrix_pred, adjacent_labels, proposal2cluster, log=True):
        ...
        img_pred_raw = draw_scatterplot(points, num_proposal=self.opt.num_proposal+1, pred=pred_centers, bbox=bbox, objectness_score=objectness_score, seg_pred=segmentation_pred)
        img_gt_raw   = draw_scatterplot(points, num_proposal=self.opt.num_proposal+1, pred=pred_centers, bbox=bbox, objectness_score=objectness_score, seg_gt=segmentation_label)
        img_pred     = draw_scatterplot(points, num_proposal=self.opt.num_proposal+1, pred=pred_centers, bbox=bbox, objectness_score=objectness_score, seg_pred=proposal2cluster[segmentation_pred])
        img_gt       = draw_scatterplot(points, num_proposal=self.opt.num_proposal+1, pred=pred_centers, bbox=bbox, objectness_label=objectness_label, seg_gt=  proposal2cluster[segmentation_label])
